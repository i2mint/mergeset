...
def test_singleton_preoracle_finds_a_change_that_cannot_land(repo):
    ...
    found = dict(
        singleton_textual_conflicts(repo, "main", {"a": "feat-a", "b": "feat-b"})
    )
    assert set(found) == {"a"}
...
def test_merged_worktree_gives_a_tree_with_every_change(repo):
    with merged_worktree(repo, "main", [("a", "feat-a"), ("b", "feat-b")]) as (
        path,
        outcome,
    ):
...
def test_conflicted_set_never_creates_a_worktree(repo):
    with merged_worktree(repo, "main", [("a", "feat-a"), ("c", "feat-c")]) as (
        path,
        outcome,
    ):
        assert path is None
...
def test_end_to_end_merge_only_analysis(repo, tmp_path):
    changes = list(
        branch_changes(repo, ["feat-a", "feat-b", "feat-c", "feat-d"], base="main")
    )
    analysis = analyze(
        repo,
        changes,
